...
/*   Updated: 2025/02/28 17:53:50 by ihancer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void free_program(t_main *program)
{
    ...
    node = program->token;  // Access the token field inside t_main
    while (node)
    {
        temp = node;
        node = node->next;
    ...
    program->token = NULL;  // Set the token field to NULL
}
...
void main_free(t_main program, char *line, int key)
{
    if (line)
        free(line);
    free_program(&program);  // Pass the address of the program itself
    if (key == 1)
        rl_clear_history();
}
